...
export default function NewReelPage() {
  ...
  const [reelVideoPreview, setReelVideoPreview] = useState<string | null>(null); // For video preview
  ...
  const handleReelFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.type.startsWith('video/')) {
        ...
        setReelVideoPreview(URL.createObjectURL(file)); // Create object URL for preview
      } else {
  ...
  const onSubmit: SubmitHandler<ReelFormValues> = async (data) => {
    ...
    try {
      ...
      const videoFilePath = `reels/${currentUser.uid}/${Date.now()}_${reelVideoFile.name}`;
      const videoFileRef = storageRefSdk(storage, videoFilePath);
      await uploadBytes(videoFileRef, reelVideoFile);
      uploadedVideoUrl = await getDownloadURL(videoFileRef);
      ...
      // 3. Call server action to create reel document
      ...
      if (result.success && result.reelId) {
        ...
        toast({
          ...
          description: result.message || "Could not save your reel. Please try again.",
          variant: "destructive",
        });
      }
    } catch (error) {
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <Button type="submit" variant="gradientPrimary" disabled={isSubmitting} className="w-full text-lg py-3">
                ...
                {isSubmitting ? "Posting Reel..." : "Post Reel"}
              </Button>
            </form>
          </Form>
        </CardContent>
